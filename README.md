...
Therefore, a small improvement to the project would be to keep the same naming convention for all the files. Either by finding a way to rename the names within `/components/ui` or changing the rest of the files to match the `shadcn/ui` naming convention.
